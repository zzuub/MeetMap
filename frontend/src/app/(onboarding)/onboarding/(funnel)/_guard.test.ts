...
/**
 * ⚠️ **판정은 폴더 단위다 — 알고 남겨 둔 한계다** (PR #36 리뷰).
 *
 * 한 단계의 소스 **어딘가에** 호출이 한 번이라도 있으면 통과한다. 그래서
 * `location/page.tsx` 의 조회 가드를 지워도 `location/_actions.ts` 에 호출이 남아
 * 있는 한 이 테스트는 조용하다 — **2차 리뷰가 찾은 `done` 누락과 같은 종류의
 * 실수**를 이 granularity 로는 전부 막지 못한다.
 *
 * **그럼에도 지금 파일 단위로 좁히지 않는다.** 좁히려면 "이 단계의 어느 파일이
 * 가드를 불러야 하는가"를 또 사람이 판단해 표로 유지해야 하는데, 그게 정확히
 * 4.51 이 깨려던 패턴이다(`NO_SESSION_STEPS` 가 하나 더 생긴다). 단계 다섯 규모
 * 에서는 폴더 단위로도 실질 위험이 낮다.
 *
 * **좁힐 시점**: 한 단계 폴더 안에 **세션이 필요한 일과 필요 없는 일이 섞이면.**
 * 지금은 단계별로 전부 아니면 전무다(`intro` 만 `NO_SESSION_STEPS`). 섞이는
 * 순간 "폴더 어딘가에 있다"가 아무것도 보장하지 않게 된다.
 */
