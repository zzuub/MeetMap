...
interface RegionSheetProps {
   ...
   *
   * 두 값이 같은 것은 우연이 아니다. 지역 조건은 다른 필터와 교차항 없는 AND 라,
   * `(전체 필터 AND 구=X)` 의 건수와 `(구 조건만 지운 스캔)` 의 X 버킷이 집합으로
   * 같다. ⚠️ 단 **둘이 같은 스냅샷을 본다는 전제**가 깔려 있다 — 목록 조회와 지역
   * 스캔은 별개 요청이라, 실 API 로 바뀌면 그 사이 데이터가 변해 근사치로 격하된다.
   * **서버 패싯으로 갈아타도 이 시간차는 안 없어진다** (4.28 · PR #25 3·4차 리뷰).
   */
  resultCount: number;
