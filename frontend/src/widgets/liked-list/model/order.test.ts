...
import { orderLikedEvents } from "./order";

/**
 * 찜 목록 순서 (9장 · `decisions.md` 4.65).
 *
...
describe("찜 목록 순서", () => {
  ...
  it("다가오는 회차를 가까운 순으로 먼저, 지난 회차는 최근 것부터 뒤에 둔다", () => {
    expect(ids(orderLikedEvents(events, now))).toEqual(expected);
  });

  it("뒤집어 넣어도 같다 — 입력 순서의 안정 정렬로 통과하지 않는다", () => {
    expect(ids(orderLikedEvents([...events].reverse(), now))).toEqual(expected);
  });

  it("오늘 이미 시작한 회차는 지난 쪽이 아니다 — 시각이 아니라 날짜를 센다", () => {
    // `date < now` 로 가르는 구현이면 `오늘저녁` 이 `어제` 앞의 지난 묶음으로 간다
    expect(ids(orderLikedEvents(events, now))[0]).toBe("오늘저녁");
  });
  ...
  it("KST 자정 직후에는 어젯밤 회차가 지난 쪽으로 간다", () => {
    ...
    const ordered = orderLikedEvents(
      [
  ...
  it("같은 시각이면 id 로 고정한다", () => {
    ...
    expect(ids(orderLikedEvents([at("나", same), at("가", same)], now))).toEqual(["가", "나"]);
  });

  it("입력을 바꾸지 않는다", () => {
    const input = Object.freeze([...events]);

    orderLikedEvents(input, now);
