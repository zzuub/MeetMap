...
export const ENDPOINTS = {
  ...
  event: {
    ...
    list: "/events",
    /**
     * 지도 마커 — **`GET /events/map?{필터}[&bbox=]`** → `EventSummary[]` (6.6).
     *
     * 요구사항 다섯.
     * 1. **같은 필터의 `GET /events` 와 같은 집합이다** (6.6 `마커 소스`). 페이지네이션이 없다
     * 2. `bbox` 는 **선택**이다. 없으면 시/도 전체(MVP1 은 서울)다 — 프론트는 지금 **보내지
     *    않는다**. 마커가 조건의 전건이라 뷰포트를 옮겨도 다시 부르지 않는다 (`decisions.md` 4.71)
     * 3. 순서는 약속하지 않아도 된다 — 지도는 순서를 그리지 않는다
     * 4. `isLiked` 는 채우지 않아도 된다 — 화면이 읽지 않는다 (4.59)
     * 5. **건수 상한이 없다 — 양쪽 다.** `getList` 는 `limit` 을 보내지만 이쪽은 안 보낸다(1번
     *    때문이다 — 조용히 자르면 리스트와 다른 집합이 된다). 그래서 **결과 수가 곧 마커 수이고
     *    DOM 오버레이 수**다. 상한을 두려면 자르는 것이 아니라 **몇 건부터 무거운지 재고**
     *    클러스터링(6.6 TBD)이나 안내 문구를 사양이 정해야 한다 (PR #48 리뷰 · `phase3-notes.md`)
     *
     * ⚠️ bbox 의 모양(`south,west,north,east` 등)은 **뷰포트 조회를 들이는 판**에 정한다 — 전건이
     * 무거워지는 시점이고, 그때가 4.58 의 번복 조건(클라이언트 조회 · 캐시)을 다시 여는 자리다.
     */
    map: "/events/map",
