...
**P1-8 은 타협 불가.** **하단 고정 CTA 를 여기서 세운다**(4.32) — 본문 하단 여백에 `--height-fixed-cta` 를 비우는 것도 같이 들어간다. 결제 비대행 고지·조건 확인 블록·하단 경고가 전부 들어가야 하고, 이 모달을 우회하는 진입 경로(지도 마커 시트의 신청 버튼 포함)를 만들지 않는다. 조건 확인 블록은 참가 연령(`N~N년생`)·모집 정원(`남 N · 여 N`)·참가비(남·여 양쪽)이며 `신청 마감` 항목은 없다.
