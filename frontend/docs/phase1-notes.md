...
- **전환 표시는 사용자가 보고 있는 자리에 붙인다.** 칩은 `useLinkStatus`(+`prefetch={false}`), 시트는 `N개 결과 보기` 가 흐려진다. route-level `loading.tsx` 는 여전히 P1-9 다

남은 것: 상단 **지역 선택 버튼**(P1-5b), 정렬 `select`·뷰 토글(P1-6), 리스트 카드의 `내 나이대 아님` 이 실제로 뜨는 것(P2-4 `viewer` 주입 — 규칙 `marksIneligible` 은 이미 있고 테스트도 있다).

**P1-5b 요점**: 지역 시트는 **시/도 → 구 2단**이고 서울만 활성이다(6.3). 구별 건수 `N곳` 이 필요한데 **목록 응답에 패싯 카운트가 없다** — 시간대 칩이 쓰는 `api/slotCounts.ts` 와 같은 문제이고, 구는 25개라 같은 방식이면 조회 25회가 된다. **서버 쪽 `facets` 를 먼저 정한다** (`progress.md` 5장). 상단 지역 선택 버튼(`📍 서울 전지역 ▾`)도 이 작업에서 만든다.
