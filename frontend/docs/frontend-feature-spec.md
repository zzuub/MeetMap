...
| 결과 수 | `총 N개 소개팅` |
| 정렬 | `select` — 인기순(기본) / 최신순 / 가격 낮은순 / 가격 높은순. **가격 정렬은 사용자 성별 기준값**(여성→`femalePrice`, 남성→`malePrice`)을 쓴다. 게스트는 가격 정렬 옵션을 숨긴다 |
| 뷰 토글 | 세그먼트 `리스트 / 지도` |
...
| 비교표 행 | 참가비(내 성별 기준) / 일정 / 거리 / 후기 / 모집 상태 (5행 고정) |
| 우위 하이라이트 | 최저가·최단거리·최고평점 셀을 강조색 + 볼드 |
| 우위 배지 | `가성비`(최저가) / `가까움`(최단거리) / `후기 좋음`(최고평점) — 소개팅당 1개, 우선순위 가성비 > 가까움 > 후기 좋음. **`가성비`는 사용자 성별 기준 가격으로 계산**한다 |
...
- 이 화면은 **소개팅별 후기 목록**과 **내가 쓴 후기** 두 용도로 읽힌다. 목업만으로는 구분 불가 → 라우트 분리 필요: `/events/[id]/reviews` vs `/my/reviews`. **TBD**
...
| 검색 대상 필드 | 소개팅명 + 지역 + 주최사 (부분 문자열 포함) |
...
| 탐색 목록 | `GET` | `/events?when&slot&scale&status&maxPrice&eligibleOnly&sort&mood&area&district&cursor` | 커서 페이지네이션. `maxPrice`·`eligibleOnly`는 서버가 인증 주체의 성별·출생연도로 해석한다 |
| 지도 마커 | `GET` | `/events/map?bbox&{필터}` | 뷰포트 기준 |
| 소개팅 상세 | `GET` | `/events/{id}` | |
