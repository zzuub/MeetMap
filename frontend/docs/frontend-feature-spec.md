...
[메인]
 홈 ──지도카드 / 지도탭──▶ 탐색(map) ──마커──▶ 마커 시트 ──▶ 소개팅 상세
 홈 ──전체보기 >────────▶ 탐색(list) ──카드──▶ 소개팅 상세
...
| 초기화 | 칩이 2개 이상이면 줄 맨 뒤에 `초기화` 텍스트 버튼. 누르면 표시 대상 전부 해제(시간대·지역은 유지) |
...
| 기능 | 동작 |
| --- | --- |
| 초기화 | 2·3층 전부 기본값으로. **1층 자격 토글은 초기화 대상이 아니다** (개인 조건이지 필터가 아니다) |
...
- 딤 클릭 시 닫힘.
- 마감된 소개팅(`status === '마감'`)의 `신청하기` 처리 — 목업 미정의, **TBD**
...
> **목업 없음.** 신규 화면이라 이 문서가 원본이다 (16장 목업 필요 목록에 등재).
...
- [ ] **목업 갱신** — 카테고리·잔여석·성비 게이지·단일 가격이 남아 있는 User v2 / Actions / My 화면
...
- [ ] **주최사 소개 페이지 `/providers/[id]`** — 2026-09-02 신설. 사양은 7.4 에 있으나 목업이 없다. 프로필 블록 배치(인스타 프로필 참고), 평점 요약 시각화, 모집 중인 회차 목록의 카드 크기
- [ ] **주최사 후기 목록 `/providers/[id]/reviews`** — 10.4 의 My 후기 목록 목업을 재사용할 수 있는지 확인 필요
