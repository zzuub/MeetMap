...
| P1-6 | 정렬 / 뷰 토글 | 6.2 | 4종 정렬 동작. **가격 정렬은 사용자 성별 기준값** 사용, 게스트는 가격 정렬 숨김 |
| P1-7 | 소개팅 상세 `/events/[id]` | 7.1·7.2 | 히어로 + 정보카드(가격 남·여 병기 / 출생연도 / 정원 / 직업군) + 하단 고정 CTA 3종. **참석자는 집계값만** — 개별 행 저장·표시 금지(7.2 원칙) |
