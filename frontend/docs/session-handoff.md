...
- **상세의 하단 고정 CTA(찜·비교 담기·신청하기)와 주최사 페이지 링크도 아직 없다** (4.32) — 같은 규칙이다. 목적지가 P2-7·P3-4·P1-8·P5-4 라 붙이면 전부 죽은 컨트롤이다. **`신청하기` 를 `externalApplyUrl` 로 직결하지 않는다** — 결제 비대행 고지(7.3)를 우회하는 경로가 된다. **바는 P1-8 이 모달과 함께 세우고** 본문 하단 여백(`--height-fixed-cta`)도 그때 넣는다
