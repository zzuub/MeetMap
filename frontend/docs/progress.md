...
| **홈 카드의 `viewer` 주입** | P1-3 완결 · P2-4 | 홈은 지금 `viewer={null}` 로 그린다 — 목 세션에 출생연도·성별이 없어서다. 그래서 로그인 상태에서도 가격이 **남·여 병기**로 나오고 섹션 B 배지가 `{N}년생` 이 아니라 모집 범위로 떨어진다. 프로필 설정(P2-4)이 붙는 자리이며, 탐색·상세도 같은 값을 필요로 한다 |
