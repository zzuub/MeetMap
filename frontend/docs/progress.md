...
| 소스 파일 | 119개 (테스트 제외) / 약 7,800줄 |
| 테스트 | 227개 (15파일), `npm test` — ⚠️ **1건 실패 중**(목 데이터 날짜 노후. 아래 5장) |
...
| ⚠️ **목 데이터의 날짜가 늙어 테스트 1건이 깨진다** | `npm test` 전체 (지금 당장) | `eventApi.mock.test.ts > 조건을 겹쳐 걸면 AND 로 좁힌다` 가 **2026-09-07 부터** 실패한다. P1-6 과 무관하고 `main`(51f019c)에서도 재현된다(stash 로 확인).<br>원인은 `MOCK_EVENTS` 의 날짜가 **절대값**(`2026-09-04` …)인데 `isThisWeek` 는 **실행 시각 기준**이라는 것이다. 9/7(월) 로 주가 넘어가면서 `THIS_WEEK` 집합이 `09-04·05·06`(SMALL 포함) → `09-12·13`(둘 다 STANDARD) 으로 갈아엎어졌고, 테스트가 쓰는 `{THIS_WEEK + OPEN + SMALL}` 조합이 **0건**이 됐다. 테스트가 "이 조건을 만족하는 목 데이터가 없다"로 정확히 실패하고 있으니 안전망은 제대로 작동한 것이다.<br>고치는 방법이 둘이라 **P1-6 범위 밖으로 둔다** — ① 목 날짜를 앞으로 밀어 경계값 배치(`phase1-notes.md` P1-0 표의 "이번 주 5건·그 이후 3건" 등)를 다시 맞춘다 → 다음 주에 또 깨진다 ② 목 날짜를 `now` 기준 **상대값**으로 만든다 → 근본 해결이지만 목이 불변이라는 전제(4.28 의 스냅샷 논의)가 흔들린다. **①/② 선택은 사용자 결정 사항** |
