...
| P1-3 | 홈 `/` | ⬜ | |
| P1-4 | 탐색 리스트 `/explore` | ⬜ | |
| P1-5 | 필터 시트 | ⬜ | **`genderPolicy` 미정의로 성별 필터 구현 불가** (5장) |
| P1-5b | 지역(구) 선택 시트 | ⬜ | |
| P1-6 | 정렬 / 뷰 토글 | ⬜ | |
| P1-7 | 행사 상세 `/events/[id]` | ⬜ | |
...
`CATEGORIES` · `AREAS` · `SEOUL_DISTRICTS` · `MOOD_TAGS` · `REVIEW_TAGS`.

**왜**: `src/README.md` 는 `shared` 를 "도메인 지식이 없는 자원"이라 정의하므로 표면적으로 충돌한다. 그러나 **옮길 곳이 없다.** `CATEGORIES` 는 event(행사 카테고리)와 user(관심 카테고리)가 함께 쓴다. `entities/event` 에 두면 `entities/user` 가 참조해야 하는데 그건 4.1 이 금지한다.
...
| 항목 | 막는 것 | 상태 |
| --- | --- | --- |
| **`genderPolicy` 필드 정의** | P1-5 성별 필터 | 미정. 목 구현에서 매칭 로직을 **의도적으로 비워둠** — 추측으로 넣으면 목업과 같은 "항상 0건" 버그를 재현하게 된다 |
...
| 초기 행사 데이터 공급 주체 | Phase 6/7 순서 | 운영자 전제로 `USER → ADMIN → PROVIDER` 진행 중 |
