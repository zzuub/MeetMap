...
| 항목 | 막는 것 | 상태 |
| --- | --- | --- |
| **목록 응답에 패싯(faceted) 카운트가 없다** | P1-5b · 서버 | 상단 시간대 칩이 **건수 0인 슬롯을 감춰야 해서**(6.2) `features/event-filter/api/slotCounts.ts` 가 슬롯마다 `totalCount` 만 받아온다 — 한 화면에 조회 4회다. **지역 선택 시트(6.3)의 구별 `N곳` 도 같은 값이 필요하다** — 25개 구면 25회가 되므로 그 전에 정한다. 실 API 목록 응답에 `facets` 가 생기면 그 함수 내부만 바뀐다(호출부는 이 함수만 본다) |
