...
### 2026-09-05 · [#](https://github.com/zzuub/MeetMap/issues) · P1-5b
**지역 선택 시트와 상단 지역 버튼**
...
| 항목 | 막는 것 | 상태 |
| --- | --- | --- |
| **목록 응답에 패싯(faceted) 카운트가 없다** | 서버 (Phase 5+) | **P1-5b 에서 방식은 정했다** → 4.28. `api/exploreFacets.ts` 가 조건에 맞는 건을 긁어와 클라이언트에서 버킷팅한다 — 조회 **2회**(축마다 1회)이고, 상한을 넘기면 그 축은 `null` 로 돌려 **건수를 포기**한다.<br>남은 것은 **서버 `facets`** 다. 목록 응답에 실려 오면 조회가 1회로 줄고 전건 스캔도 사라진다. `exploreFacets` 내부만 갈아끼우면 되고 호출부·화면 규칙은 안 바뀐다. **갈아탈 신호는 `null` 이 뜨기 시작하는 것**(스캔 상한 초과)이다 |
